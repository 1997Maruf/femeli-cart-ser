...
function verifyAdminToken(
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    res.status(401).json({
      success: false,
      error: "অননুমোদিত প্রবেশ। অনুগ্রহ করে পুনরায় লগইন করুন।",
    });
    return;
  }
  ...
  try {
    ...
    res.status(401).json({
      success: false,
      error: "সেশনের মেয়াদ শেষ হয়েছে। পুনরায় লগইন করুন।",
    });
  }
}
...
app.get("/api/survey/count", async (_req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: err?.message || "ডাটাবেস সংযোগ ব্যর্থ" });
  }
});
...
app.post("/api/survey/submit", async (req: Request, res: Response) => {
  try {
    ...
    if (!data.fullName || !data.nid || !data.mobileNumber || !data.upazila) {
      res
        .status(400)
        .json({ success: false, error: "আবশ্যকীয় তথ্যসমূহ প্রদান করুন।" });
      return;
    }
    ...
    const newSurvey: SurveyData = {
      ...
      landAmountDecimals: data.landAmountDecimals
        ? Number(data.landAmountDecimals)
        : 0,
    ...
    res.status(500).json({
      success: false,
      error: err?.message || "সংরক্ষণে ব্যর্থ হয়েছে।",
    });
  }
});
...
app.post("/api/admin/register", async (req: Request, res: Response) => {
  try {
    ...
    if (!cleanUsername || !cleanPassword) {
      res.status(400).json({
        success: false,
        error: "ব্যবহারকারীর নাম ও পাসওয়ার্ড প্রদান করা আবশ্যক।",
      });
      return;
    }
    ...
    if (cleanPassword.length < 4) {
      res
        .status(400)
        .json({ success: false, error: "পাসওয়ার্ড অন্তত ৪ অক্ষরের হতে হবে।" });
      return;
    }
    ...
    if (existingUser) {
      res.status(400).json({
        success: false,
        error:
          "এই ব্যবহারকারীর নামে ইতিমধ্যে একটি অ্যাকাউন্ট রয়েছে। লগইন করুন।",
      });
      return;
    }
    ...
    const token = jwt.sign(
      { username: cleanUsername, role: newUserRole },
      JWT_SECRET,
      { expiresIn: "24h" },
    );
    ...
    res.status(500).json({
      success: false,
      error: err?.message || "সাইন আপ ব্যর্থ হয়েছে।",
    });
  }
});
...
app.post("/api/admin/login", async (req: Request, res: Response) => {
  try {
    ...
    if (!cleanUsername || !cleanPassword) {
      res.status(400).json({
        success: false,
        error: "ব্যবহারকারীর নাম ও পাসওয়ার্ড প্রদান করুন।",
      });
      return;
    }
    ...
    if (userInDb && userInDb.password === cleanPassword) {
      ...
      const token = jwt.sign(
        { username: userInDb.username, role },
        JWT_SECRET,
        { expiresIn: "24h" },
      );
      res.json({
    ...
    const isDefaultAdmin =
      ...
      (cleanUsername.toLowerCase() === "jorip" &&
        cleanPassword === "MarufHossen1234");
    ...
    res
      .status(401)
      .json({ success: false, error: "ভুল ব্যবহারকারীর নাম অথবা পাসওয়ার্ড।" });
  } catch (err: any) {
    res.status(500).json({
      success: false,
      error: err?.message || "লগইন করতে ব্যর্থ হয়েছে।",
    });
  }
});
...
app.get(
  ...
  async (_req: AuthRequest, res: Response) => {
    try {
      ...
      const averageIncome =
        totalSubmissions > 0
          ? Math.round(grandTotalIncome / totalSubmissions)
          : 0;
      ...
      res.status(500).json({
        success: false,
        error: err?.message || "পরিসংখ্যান লোড ব্যর্থ",
      });
    }
  },
);
...
app.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: err?.message || "তালিকা লোড ব্যর্থ" });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (result.deletedCount === 1) {
        ...
        res
          .status(404)
          .json({ success: false, error: "আবেদনটি পাওয়া যায়নি।" });
      }
    } catch (err: any) {
      res.status(500).json({
        success: false,
        error: err?.message || "মুছে ফেলতে সমস্যা হয়েছে।",
      });
    }
  },
);
